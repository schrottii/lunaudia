...
function wggjLoadImages() {
    for (let image in images) {
        let img = new Image();
        img.src = wggj.config.imageBasePath + images[image];
        img.onload = () => {
            wggjLoadedImages++;
...
// classes: Base, Square, Image, Text, SmartText, Container
class WGGJ_Base {
    constructor() {
    ...
    setParent(parentName) {
        ...
        if (objects[parentName] == undefined) return false;

        this.parent = parentName;
        objects[parentName].children.push(this.objectName);
    }

    init() {
        // for button attachments
        if (this.aText != undefined) {
            createText(this.objectName + ":text", 0, 0, this.aText.text, this.aText);
            objects[this.objectName + ":text"].parentButton = this.objectName;
        }
        if (this.aImage != undefined) {
            this.aImage.centered = true;
            createImage(this.objectName + ":image", 0, 0, 0, 0, this.aImage.image, this.aImage);
            objects[this.objectName + ":image"].parentButton = this.objectName;
        }
        this.initiated = true;
    }
    ...
    buttonAttachments() {
        ...
        if (this.aImage != undefined) {
            ...
            objects[this.objectName + ":image"].w = (this.currentW() * 1) / wggj.canvas.w; // EDITED to 1
            objects[this.objectName + ":image"].h = (this.currentH() * 1) / wggj.canvas.h; // EDITED to 1
            objects[this.objectName + ":image"].render(true);
        }
        return true;
    }

    renderBase(parented = false) {
        // render disablers
        if (this.power == false) return false;
        if ((this.parent != undefined || this.parentButton != undefined) && parented === false) return false;
...
class WGGJ_Square extends WGGJ_Base {
    constructor(x, y, w, h, color, config) {
        ...
        // CONFIG
        this.objectName = config.objectName; // defined through create function
    ...
    render(parented = false) {
        if (this.clickableOnly == true) return false;
        if (this.renderBase(parented) === false) return false;
        ...
        // show hitbox, if enabled, and power is on
        if (wggj.debug.hitboxes) {
            if (this.onClick || this.onDrag || this.onHold) {
...
class WGGJ_Image extends WGGJ_Base {
...
class WGGJ_Text extends WGGJ_Base {
    constructor(x, y, text, config) {
        ...
        this.maxW = isValid(config.maxW) ? config.maxW : 0;
    ...
    renderText(renderX, renderY) {
        // single line text
        if (this.maxW !== 0) wggjCTX.fillText(this.text, renderX, renderY, this.maxW * wggj.canvas.w);
        else wggjCTX.fillText(this.text, renderX, renderY);
    }
...
class WGGJ_SmartText extends WGGJ_Text {
    constructor(x, y, text, config) {
        super(x, y, text, config);

        this.autoLinebreak = isValid(config.autoLinebreak) ? config.autoLinebreak : 0;
        this.images = config.images;
        for (let img in this.images) {
            objects[this.images[img]].power = false;
        }
    }

    dumber() {
        // removes SmartText related symbols from text 
        // useful for handling user inputs without allowing them to do things like line breaks
        this.text.replaceAll("\n", "");
        this.text.replaceAll("i{", "");
    }
    ...
    renderText(renderX, renderY) {
        ...
        for (let sT = 0; sT < splitText.length; sT++) {
            splitLine = splitText[sT];
            lineHeight = renderY + sT * this.getScaling();
            ...
            while (splitLine.includes("i{")) {
                let imageName = splitLine.split("i{")[1].split("}")[0];

                let charsBefore;
                let charsAfter;
                if (this.align == "left" || this.align == "center") charsBefore = splitLine.replaceAll("    ", "eee").replaceAll(" ", "").split("i{")[0].length;
                if (this.align == "right" || this.align == "center") charsAfter = splitLine.replaceAll("i{", "eee").replaceAll(" ", "").split(imageName + "}")[1].length + 2;

                splitLine = splitLine.replace("i{" + imageName + "}", "    "/*.repeat(this.getScaling() / 2)*/);

                objects[this.images[imageName]].w = this.getScaling() / wggj.canvas.w;
                objects[this.images[imageName]].h = this.getScaling() / wggj.canvas.h;
                if (this.align == "left") objects[this.images[imageName]].x = (renderX + charsBefore * (this.getScaling() / 2)) / wggj.canvas.w;
                if (this.align == "right") objects[this.images[imageName]].x = (renderX - charsAfter * (this.getScaling() / 2)) / wggj.canvas.w;
                if (this.align == "center") objects[this.images[imageName]].x = (renderX + charsBefore * (this.getScaling() / 2) - ((charsBefore + charsAfter) * (this.getScaling() / 2) / 2)) / wggj.canvas.w;
                objects[this.images[imageName]].y = (lineHeight - this.getScaling()) / wggj.canvas.h;
                objects[this.images[imageName]].power = true;
                objects[this.images[imageName]].render();
                objects[this.images[imageName]].power = false;
            }

            // render smart text
            if (this.maxW !== 0) wggjCTX.fillText(splitLine, renderX, lineHeight, this.maxW * wggj.canvas.w);
            else wggjCTX.fillText(splitLine, renderX, lineHeight);
        }
    }
}
...
class WGGJ_Group {
    constructor(groupname, children) {
        ...
        this.children = isValid(children) ? children : [];

        this.objectName = groupname;
    }

    addChild(childname) {
        if (objects[childname] == undefined) return false;
        this.children.push(childname);
        return true;
    }

    killChild(childname) {
        if (!this.children.includes(childname)) return false;
        this.children.splice(this.children.indexOf(childname), 1);
        return true;
    }
    ...
    set(attribute, state, condition = true, exclude = []) {
    ...
    // esentially clones of set
    add(attribute, state, condition = true, exclude = []) {
        for (let c of this.children) {
            if (exclude.includes(c)) continue;
            if (condition === true || condition(objects[c]) == true) {
                objects[c][attribute] += state;
    ...
    sub(attribute, state, condition = true, exclude = []) {
        for (let c of this.children) {
            if (exclude.includes(c)) continue;
            if (condition === true || condition(objects[c]) == true) {
                objects[c][attribute] -= state;
    ...
    mul(attribute, state, condition = true, exclude = []) {
        for (let c of this.children) {
            if (exclude.includes(c)) continue;
            if (condition === true || condition(objects[c]) == true) {
                objects[c][attribute] *= state;
    ...
    div(attribute, state, condition = true, exclude = []) {
        for (let c of this.children) {
            if (exclude.includes(c)) continue;
            if (condition === true || condition(objects[c]) == true) {
                objects[c][attribute] /= state;
...
class WGGJ_Container extends WGGJ_Group {
...
class WGGJ_RenderLayer {
    constructor(name, renderer/*, x = 0, y = 0, w = 1, h = 1*/) {
        this.name = name;
    ...
    render() {
        ...
        if (typeof (this.renderer) != "function") return false;
        this.renderer(wggj.time.delta / 1000); // ms tick
        return true;
    }
}
/*
class WGGJ_TilesLayer {
    constructor(name, verticalTiles, x = 0, y = 0, w = 1, h = 1) {
        this.name = name;
        ...
        this.tiles = []; // double array, usage like: tiles[x][y] with x and y starting at 0
        this.tilesCache = {}; // stores some vars to avoid recalculation
        ...
        this.view = [0, 0, 0, 0]; // x, y, w, h

        // required for child render to work
        this.scrolledX = 0;
        this.scrolledY = 0;
    ...
    resize(verticalTiles, x = 0, y = 0, w = 1, h = 1) {
        this.verticalTiles = verticalTiles;
        this.x = x;
        this.y = y;
        this.w = w;
    ...
    init() {
        // define size, width and height are the same
        this.tilesCache.size = this.h * wggj.canvas.h / this.verticalTiles;
        // get how many tiles per row (horizontally) based on the height / size
        this.tilesCache.horizontalTiles = Math.floor(this.w * wggj.canvas.w / this.tilesCache.size);
        // that would leave empty space to the right so center the thing
        this.tilesCache.horizontalWaste = (wggj.canvas.w * this.w) - (this.tilesCache.horizontalTiles * this.tilesCache.size);
        this.tilesCache.verticalWaste = (wggj.canvas.h * this.h) - (this.verticalTiles * this.tilesCache.size );
        this.tilesCache.offset = (this.tilesCache.horizontalWaste / 2) + (this.x * wggj.canvas.w);
        this.tilesCache.offsetY = (this.tilesCache.verticalWaste / 2) + (this.y * wggj.canvas.h);

        this.x += this.tilesCache.horizontalWaste / 2 / wggj.canvas.w;
        this.w -= this.tilesCache.horizontalWaste / wggj.canvas.w;
        this.y += this.tilesCache.verticalWaste / 2 / wggj.canvas.h;
        this.h -= this.tilesCache.verticalWaste / wggj.canvas.h;

        this.view = [0, 0, this.tilesCache.horizontalTiles, this.verticalTiles];

        //if (this.tiles.length == 0) {
        this.tiles = [];
        let xx;
        for (let x = 0; x < this.tilesCache.horizontalTiles + 2; x++) {
            xx = [];
            for (let y = 0; y < this.verticalTiles + 2; y++) {
                xx.push(new WGGJ_Image(((x + this.view[0] - 1) * this.tilesCache.size + this.tilesCache.offset) / wggj.canvas.w,
                    ((y + this.view[0] - 1) * this.tilesCache.size + this.tilesCache.offsetY) / wggj.canvas.h,
                    this.tilesCache.size / wggj.canvas.w, this.tilesCache.size / wggj.canvas.h,
                    "button", { }));
                xx[y].parent = this.objectName;
            }
            this.tiles.push(xx);
        }
    ...
    // needed to make them clickable
    objectify(x, y) {
        objects[this.objectName + ":" + x + "," + y] = this.tiles[x][y];
    }
    ...
    setVal(x, y, property, val) {
        this.tiles[x][y][property] = val;
    }

    setValAll(property, val) {
        for (let x = 0; x < this.tilesCache.horizontalTiles + 2; x++) {
            for (let y = 0; y < this.verticalTiles + 2; y++) {
                this.tiles[x][y][property] = val;
            }
    ...
    move(direction, dur) {
        // direction is where to move the tiles to 
        // for player movement, invert it 

        switch (direction) {
            case "up":
                //this.changeAll((t) => t.y -= this.tilesCache.size / wggj.canvas.w);
                createAnimation(this.objectName + ":moveUp", this.objectName, (t, d) => t.scrolledY -= this.tilesCache.size * d / dur, dur, false);
                break;
            case "right":
                //this.changeAll((t) => t.x += this.tilesCache.size / wggj.canvas.w);
                createAnimation(this.objectName + ":moveRight", this.objectName, (t, d) => t.scrolledX += this.tilesCache.size * d / dur, dur, false);
                break;
            case "down":
                //this.changeAll((t) => t.y += this.tilesCache.size / wggj.canvas.w);
                createAnimation(this.objectName + ":moveDown", this.objectName, (t, d) => t.scrolledY += this.tilesCache.size * d / dur, dur, false);
                break;
            case "left":
                //this.changeAll((t) => t.x -= this.tilesCache.size / wggj.canvas.w);
                createAnimation(this.objectName + ":moveLeft", this.objectName, (t, d) => t.scrolledX -= this.tilesCache.size * d / dur, dur, false);
                setTimeout(() => {
                    for (let x = this.tilesCache.horizontalTiles + 1; x >= 0; x--) {
                        for (let y = this.verticalTiles + 1; y >= 0; y--) {
                            console.log(x, y);
                            if (this.tiles[x + 1] != undefined) this.tiles[x + 1][y] = this.tiles[x][y];
                            else this.tiles[0][y] = this.tiles[x][y];
                        }
                    }
    ...
    render() {
        ...
        if (wggj.debug.hitboxes) {
            wggjCTX.fillStyle = "red";
            wggjCTX.fillRect(
                this.ogX * wggj.canvas.w,
                this.ogY * wggj.canvas.h,
                this.ogW * wggj.canvas.w,
                this.ogH * wggj.canvas.h
            );
            wggjCTX.fillStyle = "black";
            wggjCTX.fillRect(
                this.x * wggj.canvas.w,
                this.y * wggj.canvas.h,
                this.w * wggj.canvas.w,
                this.h * wggj.canvas.h
            );
        }
...
class WGGJ_Animation {
    ...
    durationTick(delta, i) {
        this.dur += delta;
        //this.sec = Math.floor(this.dur / 1000);
...
function createSmartText(name, x, y, text, config = {}) {
    if (objects[name] == undefined) {
        config.objectName = name;
        objects[name] = new WGGJ_SmartText(x, y, text, config);
        return name;
    }
    return "";
}
...
function createClickable(clickableName, x, y, w, h, onClick, config = {}) {
    if (objects[clickableName] == undefined) {
        config.objectName = clickableName;
...
function createGroup(groupname, children) {
    if (groups[groupname] == undefined) {
        groups[groupname] = new WGGJ_Group(groupname, children);
        return groupname;
    }
    return "";
}
...
function createRenderLayer(name, renderer) {
    if (objects[name] == undefined) {
        objects[name] = new WGGJ_RenderLayer(name, renderer);
        return name;
    }
    return "";
}

function createTilesLayer(name, verticalTiles, x = 0, y = 0, w = 1, h = 1) {
    if (objects[name] == undefined) {
        objects[name] = new WGGJ_TilesLayer(name, verticalTiles, x, y, w, h);
        objects[name].init();
        return name;
    }
    return "";
...
function isValid(v) {
    if (v == undefined || v === "" || (v.length != undefined && v.length == 0 && typeof (v) != "function")) return false;
    return true;
}
